...
d_coarse = 76.5e-3;
sigmaCode = 10e-9;
clockBiasMags = 240; % seconds
posAssistErrorMags = 20000:10000:150000; % meters
niter = 6;
n_epochs = 20;

if MEASX
    scenario_n_files = {'rinex_download/tlv/bshm1330.20n'};
    
    scenario_o_files = {'rinex_download/tlv/ubxout_2.txt'};
else
    scenario_n_files = {'rinex_download/tlv/bshm0400.20n'};
    
    scenario_o_files = {'rinex_download/tlv/ubxout.txt'};
end

gt_per_file_lla = ...
    [  32.1121884  34.8055935  77.304];
...
for i_cbm = 1:numel(clockBiasMags)
    for i_paem = 1:numel(posAssistErrorMags)
        ...
        for file_idx = 1%1:N_files
            ...
            if MEASX
                epochs = ubx_reader_codephase(ofile1);
            else
                epochs = ubx_reader(ofile1);
            end
            ...
            for ne = 1:n_epochs
                ...
                if MEASX
                    obs1 = epochs(ne).obs.cp';
                else
                    obs1 = epochs(ne).obs.pr';
                end
...
%                 ranges = obs1; % assume obs1 is range (almost true in these recordings)
%                 truNs = floor(ranges / (c*tcode));
                    gps_codephases = mod(obs1 / (c*tcode), 1); % [0,1]
                end
                ...
                fprintf('Our Diggelen - locErr: %.5f resNorm: %.5f, clockErr: %e, betaErr: %e\n', ...
                    locationErr, resnorm, clockErr, BetaErr);
...
%                 disp('ILS approach');
                ...
                if ~NEW_ILS_ALGO
                    ...
                    %true importance of this stage is nhat and not X_smils
                end
...
%                 results.ILS_intErr = max( abs((truNs-truNs(1)) - (nhat-nhat(1))) );
                ...
                fprintf('ILS: locErr: %.5f, clockErr: %e, betaErr: %e\n', ...
                    results.ILS_locationErr, results.ILS_clockErr, BetaErr);
